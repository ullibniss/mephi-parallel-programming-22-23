...
int main(int argc, char** argv) {
    ...
    const int threads = 6;         ///< Number of parallel threads to use
...
//    for(int i=0; i<count; i++) { array[i] = rand(); }
    ...
    for (int procs = 1; procs <= omp_get_num_procs(); procs++) {
        if (procs == 1) {
...
#pragma omp parallel num_threads(procs) shared(array, count) reduction(max: max) default(none)
            {
#pragma omp for
                for (int i = 0; i < count; i++) {
